...
// Name of this script
scriptName "BIS_fnc_EGSpectator_qbmod";
...
switch (_mode) do
{
	...
	case "Initialize" :
	{
		...
		if (!_bIsInitialized && _isPlayer) then
		{
            ...
            if (count _whitelistedUnits == 0) then {
                {
                    _units pushBack _x;
                } forEach allUnits;
            };
            ...
            missionNamespace setVariable [VAR_WHITELISTED_UNITS, _units];
	...
	case "Terminate" :
	{
		...
		if (["IsInitialized"] call SPEC) then
		{
            ...
            missionNamespace setVariable [VAR_WHITELISTED_UNITS, nil];
	...
	case "GetTargetEntities" :
	{
        ...
        private _whitelistUnits = missionNamespace getVariable [VAR_WHITELISTED_UNITS, []];
		private _whitelistEmpty = count _whitelist < 1;
        private _whitelistUnitsEmpty = count _whitelistUnits < 1;
		private _entities = [];
		...
		// Side filter
		{
			...
			(
                ...
                { (_whitelistUnitsEmpty || { _x in _whitelistUnits }) }
			) then
			{
	...
	case "GetTargetGroups" :
	{
		...
		private _whitelistUnits = missionNamespace getVariable [VAR_WHITELISTED_UNITS, []];
		private _whitelistEmpty = count _whitelist < 1;
		private _whitelistEmptyUnits = count _whitelistUnits < 1;
		private _groups = [];
		...
		{
			if ((_whitelistEmpty || { side _x in _whitelist }) && (_whitelistUnitsEmpty || { {_x in _whitelistUnits } count (units _x) > 0 }) && {{!(_x isKindOf SPECTATOR_CLASS)} count units _x > 0 }) then
			{
